...
#include "../idl/StartRequestBInfrared/StartRequestBInfrared.h"
...
StartRequestRadar_radar_info *receiveData_0735104(dds_entity_t participant, dds_entity_t topic, dds_entity_t reader);

StartRequestBInfrared_infrared_info *receiveData_2453475(dds_entity_t participant, dds_entity_t topic, dds_entity_t reader);



endResponseSF_release_result end_6460621;
void sendData_6460621(dds_entity_t participant, dds_entity_t topic, dds_entity_t writer, endResponseSF_release_result data);
...
int main(int argc, char **argv)
{
    ...
    startInfrared = dds_create_topic(participant, &StartRequestBInfrared_infrared_info_desc, "startInfrared", NULL, NULL);
    if (startInfrared < 0)
    {
...
while(1){
    ...
    StartRequestRadar_radar_info *start_0735104 = receiveData_0735104(participant, startRadar, startRadar_reader);
    
    StartRequestBInfrared_infrared_info *start_2453475 = receiveData_2453475(participant, startInfrared, startInfrared_reader);
    ...
    radar_info request_7720588;
    
    infrared_info request_227781;
    
    iamge_input request_8115402;
    
    fusion_input request_8090227;
    
    detect_request request_3907885;
    
    range_request request_7373506;
    
    lock_request request_2206985;
    
    release_request request_5022442;
    
    
    //为开始的服务赋值
    
    request_7720588.x = start_0735104->x;
    
    request_7720588.y = start_0735104->y;
    
    request_7720588.z = start_0735104->z;
    
    request_7720588.size = start_0735104->size;
    
    request_227781.x = start_2453475->x;
    
    request_227781.y = start_2453475->y;
    
    request_227781.z = start_2453475->z;
    
    request_227781.size = start_2453475->size;
    ...
    radar_result reply_7720588 = atomic_attack_radar->getRaderMonitor(request_7720588);
    
    infrared_result reply_227781 = atomic_attack_infrared->getInfraredMonitor(request_227781);
    


    // 赋值
    
    request_8090227.size = reply_7720588.size;
    
    request_8090227.x = reply_7720588.x;
    
    request_8090227.y = reply_7720588.y;
    
    request_8090227.z = reply_7720588.z;
    
    request_8115402.size = reply_227781.size;
    
    request_8115402.x = reply_227781.x;
    
    request_8115402.y = reply_227781.y;
    
    request_8115402.z = reply_227781.z;
    
    // 运行服务
    
    iamge_output reply_8115402 = atomic_attack_decode->decodeImage(request_8115402);
    


    // 赋值
    
    request_8090227.x = reply_8115402.x;
    
    request_8090227.y = reply_8115402.y;
    
    request_8090227.z = reply_8115402.z;
    
    request_8090227.size = reply_8115402.size;
    
    // 运行服务
    
    fusion_output reply_8090227 = atomic_attack_fusion->getFusionData(request_8090227);
    


    // 赋值
    
    request_3907885.x = reply_8090227.x;
    
    request_3907885.y = reply_8090227.y;
    
    request_3907885.z = reply_8090227.z;
    
    request_3907885.size = reply_8090227.size;
    
    request_7373506.x = reply_8090227.x;
    
    request_7373506.y = reply_8090227.y;
    
    request_7373506.z = reply_8090227.z;
    
    request_7373506.size = reply_8090227.size;
    
    // 运行服务
    
    detect_result reply_3907885 = atomic_attack_detect->imageDetect(request_3907885);
    
    range_result reply_7373506 = atomic_attack_range->imageRange(request_7373506);
    


    // 赋值
    
    request_2206985.object_class = reply_3907885.object_class;
    
    request_2206985.confidence = reply_3907885.confidence;
    
    request_2206985.x_min = reply_3907885.x_min;
    
    request_2206985.y_min = reply_3907885.y_min;
    
    request_2206985.x_max = reply_3907885.x_max;
    
    request_2206985.y_max = reply_3907885.y_max;
    
    request_2206985.z = reply_7373506.z;
    
    // 运行服务
    
    lock_result reply_2206985 = atomic_attack_lock->targetLock(request_2206985);
    


    // 赋值
    
    request_5022442.x = reply_2206985.x;
    
    request_5022442.y = reply_2206985.y;
    
    request_5022442.z = reply_2206985.z;
    
    // 运行服务
    
    release_result reply_5022442 = atomic_attack_release->getTargetInfo(request_5022442);
    


    // 赋值
    
    std::cout << "fj_front output: " << std::endl
    
            << reply_5022442.x << std::endl
    
            << reply_5022442.y << std::endl
    
            << reply_5022442.z << std::endl
    
            << reply_5022442.return_type << std::endl;

    // 赋值
    
    end_6460621.x = reply_5022442.x;
    
    end_6460621.y = reply_5022442.y;
    
    end_6460621.z = reply_5022442.z;
    
    end_6460621.return_type = reply_5022442.return_type;
    
    
    sendData_6460621(participant, endSF, endSF_writer, end_6460621);
...
StartRequestBInfrared_infrared_info *receiveData_2453475(dds_entity_t participant, dds_entity_t topic, dds_entity_t reader)
{
    // 定义dds数据类型
    dds_return_t rc;
    StartRequestBInfrared_infrared_info *msg;
    void *samples[1];
    dds_sample_info_t infos[1];
    ...
    for (int i = 0; i < 1; i++)
    {
        samples[i] = StartRequestBInfrared_infrared_info__alloc();
    }
    ...
    while (true)
    {
        ...
        {
            // 显式类型转换
            msg = (StartRequestBInfrared_infrared_info *)samples[0];
            printf("=== [Subscriber] Received: ");
            //printf("Message (number1: %d, number2: %d, doubleNumber1: %f, doubleNumber2: %f, message1: %s, message2: %s)\n",
            //       msg->number1, msg->number2, msg->doubleNumber1, msg->doubleNumber2, msg->message1, msg->message2);
            // 强制刷新标准输出缓冲区，确保消息立即显示在控制台上
            ...
            // 如果需要返回消息，可以在这里复制数据
            StartRequestBInfrared_infrared_info *ret_msg = (StartRequestBInfrared_infrared_info *)malloc(sizeof(StartRequestBInfrared_infrared_info));
            if (ret_msg == NULL)
            {
                DDS_FATAL("Failed to allocate memory for return message\n");
                // 删除参与者
