...
#include "../../atomic_service/atomic_test_light/sync_client_impl/atomic_test_light_sync_client_impl.cpp"
...
light_result getLightMonitor(light_info request);
...
int main(int argc, char **argv)
{
    ...
    // 声明对象
    atomic_test_light_Service_Client *atomic_test_light = nullptr;

    for (int i = 0; i < channel_list.size(); i++)
    {
        if (channel_list[i].ip == queryInfo[0].ip + ":" + std::to_string(queryInfo[0].port))
        {
            atomic_test_light = new atomic_test_light_Service_Client(channel_list[i].channel);
    ...
    for (int i = 1; i <= count; i++)
    {
        ...
        light_result reply = atomic_test_light->getLightMonitor(request);
