...
#include "../../atomic_service/atomic_test_zlsc/sync_client_impl/atomic_test_zlsc_sync_client_impl.cpp"
...
command atomic_service_fun_task_A(state_info request);
...
int main(int argc, char **argv)
{
    ...
    // 声明对象
    atomic_test_zlsc_Service_Client *atomic_test_zlsc = nullptr;

    for (int i = 0; i < channel_list.size(); i++)
    {
        if (channel_list[i].ip == queryInfo[0].ip + ":" + std::to_string(queryInfo[0].port))
        {
            atomic_test_zlsc = new atomic_test_zlsc_Service_Client(channel_list[i].channel);
    ...
    for (int i = 1; i <= count; i++)
    {
        ...
        command reply = atomic_test_zlsc->atomic_service_fun_task_A(request);

        std::cout << "reply: signal = " << reply.signal << std::endl;
        ...
        reply = atomic_service_fun_task_A(request);
        std::cout << "reply: signal = " << reply.signal << std::endl;
        std::cout << "reply: return_type = " << reply.return_type << std::endl;
