...
#include "../../atomic_service/atomic_test_infrared/sync_client_impl/atomic_test_infrared_sync_client_impl.cpp"
...
infrared_result getInfraredMonitor(infrared_info request);
...
int main(int argc, char **argv)
{
    ...
    // 声明对象
    atomic_test_infrared_Service_Client *atomic_test_infrared = nullptr;

    for (int i = 0; i < channel_list.size(); i++)
    {
        if (channel_list[i].ip == queryInfo[0].ip + ":" + std::to_string(queryInfo[0].port))
        {
            atomic_test_infrared = new atomic_test_infrared_Service_Client(channel_list[i].channel);
    ...
    for (int i = 1; i <= count; i++)
    {
        ...
        // 运行远程服务
        std::cout << "=====remote_start_time:" << getSystemMicroseconds() << std::endl;
        infrared_result reply = atomic_test_infrared->getInfraredMonitor(request);
        ...
        std::cout << "=====remote_end_time:" << getSystemMicroseconds() << std::endl;

        // 运行本地服务
        std::cout << "=====local_start_time:" << getSystemMicroseconds() << std::endl;
        reply = getInfraredMonitor(request);
        std::cout << "reply: x = " << reply.x << std::endl;
        std::cout << "reply: y = " << reply.y << std::endl;
        std::cout << "reply: z = " << reply.z << std::endl;
        std::cout << "reply: size = " << reply.size << std::endl;
        std::cout << "reply: return_type = " << reply.return_type << std::endl;
        std::cout << "=====local_end_time:" << getSystemMicroseconds() << std::endl;
