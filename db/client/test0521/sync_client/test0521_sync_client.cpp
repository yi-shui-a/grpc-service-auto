...
#include "../../../atomic_service/atomic_service_ycl/sync_client_impl/atomic_service_ycl_sync_client_impl.cpp"

#include "../../../atomic_service/atomic_service_zlsc/sync_client_impl/atomic_service_zlsc_sync_client_impl.cpp"

#include "../../../atomic_service/RobotPose/sync_client_impl/RobotPose_sync_client_impl.cpp"

#include "../../../atomic_service/FusionData/sync_client_impl/FusionData_sync_client_impl.cpp"
...
#include "../../../atomic_service/ImageDetection/sync_client_impl/ImageDetection_sync_client_impl.cpp"
...
#include "../idl/EndRequest/EndRequest.h"
...
StartRequest_atomic_service_mbsb_task_A_Request_st *receiveData_7736173(dds_entity_t participant, dds_entity_t topic, dds_entity_t reader);



EndRequest_atomic_service_sf_task_E_Reply_st end_5086696;
void sendData_5086696(dds_entity_t participant, dds_entity_t topic, dds_entity_t writer, EndRequest_atomic_service_sf_task_E_Reply_st data);
...
int main(int argc, char **argv)
{
    ...
    // 定义返回值列表，依次查询服务信息
    QueryInfo queryInfo[7];
    // 查询服务信息
    queryServiceInfo(serviceName, num, 7, queryInfo, broadcastAddress, broadcastPort);
    ...
    atomic_service_ycl_Service_Client *atomic_service_ycl = nullptr;
    
    atomic_service_zlsc_Service_Client *atomic_service_zlsc = nullptr;
    
    RobotPose_Service_Client *RobotPose = nullptr;
    
    FusionData_Service_Client *FusionData = nullptr;
    
    atomic_service_mbsb_Service_Client *atomic_service_mbsb = nullptr;
    
    atomic_service_sf_Service_Client *atomic_service_sf = nullptr;
    
    ImageDetection_Service_Client *ImageDetection = nullptr;
    ...
    for (int i = 0; i < channel_list.size(); i++)
    {
        ...
        {
            ImageDetection = new ImageDetection_Service_Client(channel_list[i].channel);
    ...
    // 创建Topic
    dds_entity_t endA;
    endA = dds_create_topic(participant, &EndRequest_atomic_service_sf_task_E_Reply_st_desc, "endA", NULL, NULL);
    if (endA < 0)
    {
...
while(1){
    ...
    StartRequest_atomic_service_mbsb_task_A_Request_st *start_7736173 = receiveData_7736173(participant, startA, startA_reader);
    ...
    input_info request_2358994;
    
    state_info request_8500560;
    
    RequestSignal_2 request_1913170;
    
    FusionInput request_5154493;
    
    atomic_service_mbsb_task_A_Request_st request_7803121;
    
    atomic_service_sf_task_E_Request_st request_1695920;
    
    RequestSignal request_708486;
    
    
    //为开始的服务赋值
    
    request_2358994.number1 = start_7736173->number1;
    
    request_2358994.number2 = start_7736173->number2;
    ...
    process_result reply_2358994 = atomic_service_ycl->atomic_service_fun_task_E(request_2358994);
    


    // 赋值
    
    request_8500560.x = reply_2358994.x;
    
    request_8500560.y = reply_2358994.y;
    
    request_8500560.z = reply_2358994.z;
    
    // 运行服务
    
    command reply_8500560 = atomic_service_zlsc->atomic_service_fun_task_A(request_8500560);
    


    // 赋值
    
    request_1913170.signal = reply_8500560.signal;
    
    request_708486.signal = reply_8500560.signal;
    
    // 运行服务
    
    RobotResult reply_1913170 = RobotPose->getRobotPose(request_1913170);
    
    DetectionResult reply_708486 = ImageDetection->imageDetect(request_708486);
    


    // 赋值
    
    request_5154493.robot_id = reply_1913170.robot_id;
    
    request_5154493.camera_id = reply_708486.camera_id;
    
    // 运行服务
    
    FusionOutput reply_5154493 = FusionData->getFusionData(request_5154493);
    


    // 赋值
    
    request_7803121.number1 = reply_5154493.number1;
    
    request_7803121.number2 = reply_5154493.number2;
    
    // 运行服务
    
    atomic_service_mbsb_task_A_Reply_st reply_7803121 = atomic_service_mbsb->atomic_service_fun_task_A(request_7803121);
    


    // 赋值
    
    request_1695920.sum_result = reply_7803121.sum_result;
    
    // 运行服务
    
    atomic_service_sf_task_E_Reply_st reply_1695920 = atomic_service_sf->atomic_service_fun_task_E(request_1695920);
    


    // 赋值
    
    std::cout << "test0521 output: " << std::endl
    
            << reply_1695920.sum_result << std::endl
    
            << reply_1695920.return_type << std::endl;

    // 赋值
    
    end_5086696.sum_result = reply_1695920.sum_result;
    
    end_5086696.return_type = reply_1695920.return_type;
    
    
    sendData_5086696(participant, endA, endA_writer, end_5086696);
