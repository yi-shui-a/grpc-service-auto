#include <iostream>
#include <dds/dds.h>
#include "EndResponse.h"  // IDL生成的C语言头文件
...
int main() {
    ...
    topic = dds_create_topic(
        participant,
        &EndResponse_atomic_service_sf_task_E_Reply_st_desc,  // IDL生成的类型描述
        "endA",  // 主题名称（必须与发布者一致）
        NULL, NULL
    );
    if (topic < 0) {
    ...
    while (1) {
        ...
        samples[0] = EndResponse_atomic_service_sf_task_E_Reply_st__alloc();
        if (samples[0] == NULL) {
        ...
        if (rc < 0) {
            ...
            EndResponse_atomic_service_sf_task_E_Reply_st_free(samples[0], DDS_FREE_ALL);
            cleanup();
            return EXIT_FAILURE;
        }
        ...
        if (rc > 0 && info.valid_data) {
            // 转换为目标结构体类型
            EndResponse_atomic_service_sf_task_E_Reply_st *data = 
                (EndResponse_atomic_service_sf_task_E_Reply_st *)samples[0];

            // 使用cout打印数据
            std::cout << "==============================" << std::endl;
            std::cout << "接收到新数据:" << std::endl;
            std::cout << "sum_result: " << data->sum_result << std::endl;
