...
#include <random>
#include <dds/dds.h>
#include "StartRequestInfrared.h" // IDL生成的C语言头文件
...
int main()
{
    ...
    topic = dds_create_topic(
        participant,
        &StartRequestInfrared_infrared_info_desc, // IDL生成的类型描述
        "startInfrared",                          // 主题名称（必须与订阅者一致）
        NULL, NULL);
    if (topic < 0)
    {
    ...
    for (int i = 0; i < 10000; i++)
    {
        ...
        StartRequestInfrared_infrared_info data;
        data.x = dist(rng);
        data.y = dist(rng);
        data.z = dist(rng);
        data.size = dist(rng);
