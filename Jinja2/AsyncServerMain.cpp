...
private:
    ...
    std::string port;
public:
    ServerImpl(std::string ip, std::string port) : ip(ip), port(port) {}
    ~ServerImpl() {
        server_->Shutdown();
...
int main(int argc, char** argv) {
    ...
    std::cout << ">>>> Broadcast Port: " << broadcastPort << std::endl;
    ...
    /* Create ServerImpl BEGAIN */
    ServerImpl serverPersonal(ip, port);
    serverPersonal.Run();
    
    std::thread atomic_service_sfzt_thread(&ServerImpl::atomic_service_sfzt_HandleRpcs, &serverPersonal);
    
    std::thread atomic_service_skzt_thread(&ServerImpl::atomic_service_skzt_HandleRpcs, &serverPersonal);
    
    std::thread atomic_service_mbsb_thread(&ServerImpl::atomic_service_mbsb_HandleRpcs, &serverPersonal);
    
    std::thread atomic_service_sj_thread(&ServerImpl::atomic_service_sj_HandleRpcs, &serverPersonal);
    
    std::thread atomic_service_st_thread(&ServerImpl::atomic_service_st_HandleRpcs, &serverPersonal);
